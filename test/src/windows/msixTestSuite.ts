...
export function registerMsixTests(toolsets: ToolsetConfig): void {
  ...
  test("msix startMenuGroup manifest contains desktop7 element", ({ expect }) =>
    app(expect, {
      targets: target,
      config: {
        toolsets,
        msix: {
          startMenuGroup: "My App Suite",
        },
        appxManifestCreated: async filepath => {
          const fileContent = await readFile(filepath, "utf-8")
  ...
  test("msix sharedPackageContainer manifest contains desktop9 element", ({ expect }) =>
    app(expect, {
      targets: target,
      config: {
        toolsets,
        msix: {
          sharedPackageContainer: {
            name: "MySharedContainer",
            memberPackages: ["com.example.App2_abcde12345"],
          },
        },
        appxManifestCreated: async filepath => {
          const fileContent = await readFile(filepath, "utf-8")
