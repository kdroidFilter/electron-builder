...
// The native (non-VM) MSIX install path runs Add-AppxPackage, which mutates machine cert stores
// and the per-user AppX package registry and is unreliable on shared/Windows-Server CI runners.
// It is opt-in via RUN_MSIX_INSTALL_TESTS (mirroring RUN_SNAP_TESTS / DO_APPX_CERT_STORE_AWARE_TEST).
