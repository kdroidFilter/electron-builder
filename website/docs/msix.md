...
| Shared Package Container (`desktop9`) | No | Yes (`sharedPackageContainer`) |
| App Migration / Start menu (`desktop7`) | No | Yes (`startMenuGroup`) |
...
- **You need modern packaging features** — package integrity, bundled architectures, registered Windows services, or shared package containers.
...
MSIX packaging requires a modern Windows Kits toolset that bundles `makeappx.exe`, `makepri.exe`, and the MSIX-capable Windows SDK. Set [`toolsets.winCodeSign`](configuration.md) to `1.0.0` or `1.1.0`:
...
:::note[Apostrophes in the publisher]
The publisher string is emitted inside a single-quoted XML attribute. An apostrophe (`'`) in a certificate Subject (e.g. `O='Bob's Tools'`) is a known limitation of the current template. Prefer a Subject without apostrophes.
:::
...
When enabled, emits `<uap10:PackageIntegrity Level="turnOn" />` inside `<Properties>`. Windows then verifies that no package files have been tampered with after signing. **Requires Windows 10 version 2004 (build 19041) or later.**

## Windows Services

Register Windows Services that ship inside the package (`desktop6` schema). **Requires Windows 10 version 2004 or later.**
...
msix:
  windowsServices:
    - name: "MyBackgroundService"        # required — Service Control Manager name
      ...
      startupType: "auto"                # auto | manual | systemGuarded. Default: manual
...
| `startupType` | No | `manual` | How Windows starts the service: `auto`, `manual`, or `systemGuarded`. |
| `arguments` | No | — | Command-line arguments passed to the service executable. |

Each entry produces a `<desktop6:Extension Category="windows.service">` declaration.

## Shared Package Container

Allow multiple packages to share a single application container (`desktop9` schema). **Requires Windows 11.**

```yaml
msix:
  sharedPackageContainer:
    name: "MySharedContainer"            # required
    memberPackages:
      - "com.example.App2_abcde12345"    # package family names of member packages
```

Produces a `<desktop9:SharedPackageContainer>` element at the `<Package>` level.

## Start Menu Group

```yaml
msix:
  startMenuGroup: "My App Suite"   # Requires Windows 11
```

Emits a `<desktop7:Extension Category="windows.appMigration">` declaration (`desktop7` schema). This associates the package with the supplied identifier for Start-menu/app-migration purposes.

:::note[Beta behavior]
The exact runtime effect of `startMenuGroup` depends on the Windows app-migration mechanism and may not visibly group apps on all Windows 11 builds. Treat this option as experimental.
:::
...
The custom manifest supports the same `${}` macros as the [default template](https://github.com/electron-userland/electron-builder/blob/master/packages/app-builder-lib/templates/msix/appxmanifest.xml), including MSIX-specific macros `${packageIntegrity}` and `${sharedPackageContainer}`. Windows services and the start-menu group are injected via the `${extensions}` macro alongside protocol/file-association extensions.

:::warning[Namespaces in a custom template]
If you supply your own template, it must declare the namespaces used by the features you enable — `uap10` (package integrity), `desktop6` (services), `desktop7` (start menu), and `desktop9` (shared container) — and list the appropriate prefixes in `IgnorableNamespaces`. Omitting them produces an invalid manifest. Start from the default template above.
:::
...
**Which toolset version do I need?** `toolsets.winCodeSign` must be `1.0.0` or `1.1.0`. The legacy `2.6.0` bundle lacks the required SDK.
